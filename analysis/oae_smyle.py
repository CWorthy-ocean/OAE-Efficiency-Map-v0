...
# # this save cesm-cases to /glade/work
...
# this save cesm-cases to /glade/scratch
caseroot_root = f"{config.dir_project_root_scratch}/cesm-cases"
os.makedirs(caseroot_root, exist_ok=True)
...
refcaserest_root = "/glade/scratch/mclong/SMYLE-FOSI/rest-reset-alt-co2"
...
def create_clone(
    ...
    def xmlchange(arg):
        ...
        check_call(["./xmlchange"]+arg.split(" "), cwd=caseroot)
    ...
    # change st_archive to share queue
    xmlchange(f"--subgroup case.st_archive JOB_QUEUE=share")
    xmlchange(f"--subgroup case.st_archive JOB_WALLCLOCK_TIME=01:00:00")
    
    
    #xmlchange(f"JOB_WALLCLOCK_TIME=01:00:00 --subgroup case.st_archive")
    #xmlchange(f"JOB_QUEUE=economy --subgroup case.run")
    #xmlchange(f"JOB_WALLCLOCK_TIME={wallclock} --subgroup case.run")
...
def open_dataset(case, stream='pop.h', from_run_dire=False, from_campaign=False):
